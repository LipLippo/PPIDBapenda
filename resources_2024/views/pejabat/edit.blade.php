    ...
    <div class="row layout-top-spacing">
            ...
            <div class="statbox widget box box-shadow">
                ...
                <div class="widget-content widget-content-area">
                   {!! Form::model($pejabat, ['method' => 'PATCH','route' => ['sotk_pejabat.update', $pejabat->id],'enctype'=>'multipart/form-data']) !!}
                        {{ Form::hidden('id') }}
                        ...
                        <div class="form-group mb-0">
                            ...
                            <select name="idjab" id="" class="form-control basic">
                                ...
                                @foreach ($sotk as $prn)
                                 {{ $s = ($prn->id == $pejabat->idjab)?' selected ':''; }}
                            ...
                            <select name="idgol" id="" class="form-control basic">
                                <option value="0">::Pilih::</option>
                                @foreach ($gol as $x)
                                 {{ $s = ($x->idgolru == $pejabat->idgol)?' selected ':''; }}
                                <option {{ $s }} value="{{ $x->idgolru }}">{{ $x->golru }} | {{ $x->pangkat }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group mb-2 mt-0">
                            ...
                            <img width="150px" src="{{ url('packages/upload/pejabat/'.$pejabat->photo) }}"/>
                             
                        </div>
                        <div class="form-group mb-4">
                            <label for="inputCity">Status</label>
                            <select class="form-control  basic" name="flag" required>
                                <option {{ $pejabat->flag == 1 ? 'selected' : '' }} value="1">Publish</option>
                                <option {{ $pejabat->flag == 0 ? 'selected' : '' }} value="0">Tidak Publish</option>
                            </select>
                        </div>
                       
                        <input type="submit" name="" class="mt-4 mb-4 btn btn-primary">
                        <a class="mt-4 mb-4 btn btn-secondary" href="{{ route('sotk_pejabat.index') }}"> Back</a>
                    {!! Form::close() !!}
                </div>
            </div>
        </div>
