          ...
          <div class="statbox widget box box-shadow">
            <a class="btn btn-success" href="{{ route('videos.create') }}"> Create New Video</a>
              <div class="widget-content widget-content-area">
                  <table id="zero-config" class="table style-3  table-hover">
                      <thead>
                            <tr>
                                ...
                                <th rowspan="2" width="15%">Foto</th>
                                 <th rowspan="2" width="15%">Url</th>
                                <th rowspan="2" width="7%">Order</th>
                                <th colspan="3" width="7%" class="text-center">Status</th>
                      ...
                      <tbody id="tablecontents">
                        @foreach ($video as $keyurl => $al)
                            <tr id="tr_{{$al->id}}" class="row1" data-id="{{ $al->id }}">
                                <td> <input type="checkbox" class="sub_chk" data-id="{{$al->id}}">  </td>
                                <td><i class="fa-solid fa-arrows-up-down-left-right"></i> {{ $al->keterangan }}</td>
                                <td><img src="{{ url('packages/upload/galeri/'.$al->foto ) }}" alt="" height="50px" width="50px"></td> 
                                <td>{{ $al->url }}</td>
                                <td>{{ $al->order }}</td>
                                <td>
                                ...
                                <td >
                                            ...
                                            <a class="btn btn-primary btn-sm" href="{{ route('videos.edit',$al->id) }}">Edit</a>
                          ...
                          <div id="rotateleftModal{{ $al->id }}" class="modal animated rotateInDownLeft custo-rotateInDownLeft" role="dialog">
                                <div class="modal-dialog">
                                    <!-- Modal content-->
                                     <form action="{{ route('videos.destroy',$al->id) }}" method="POST">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                    <p class="modal-text">Anda yakin ingin menghapus video {{ $al->keterangan }}</p>
                                            </div>
                                            <div class="modal-footer md-button">
                                                <button class="btn" data-dismiss="modal"><i class="flaticon-cancel-12"></i> Discard</button>
                                                @csrf
                                                @method('DELETE')
                                                @can('mod-video-delete')
                  ...
                  <button style="display: none" id="btndlt" class="btn btn-warning btn-sm delete_all" data-url="{{ url('video-multidel') }}">Hapus data</button>
                  {{-- {!! $data->render() !!} --}}
              </div>
          </div>
      </div>
