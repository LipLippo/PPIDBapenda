...
<br><div class="layout-px-spacing">
  <div class="seperator-header">
      <h4 class="">Realisasi Pendapatan</h4>
  </div>
  ...
  <div class="row layout-spacing">
      ...
      <div class="col-lg-12">
          ...
          <div class="statbox widget box box-shadow">
            <a class="btn btn-success" href="{{ route('statizin.create') }}"> Create New</a>
              <div class="widget-content widget-content-area">
                  <table id="zero-config" class="table style-3  table-hover">
                      ...
                      <tbody id="tablecontents">
                        @foreach ($perizinan as $key => $al)
                            <tr class="row1" data-id="{{ $al->id }}">
                                 <td><input type="checkbox" class="sub_chk" data-id="{{$al->id}}"></td>
                                <td>{{ $al->title }}</td>
                                <td>{{ $al->xaxis }}</td>
                                <td>{{ $al->yaxis }} | {{ $al->pangkat }}</td>
                                <td>{{ $al->tanggal }} </td>
                               
                                
                                <td >
                                   
                                            <a class="btn btn-primary btn-sm" href="{{ route('statizin.edit',$al->id) }}">Edit</a>
                          ...
                          <div id="rotateleftModal{{ $al->id }}" class="modal animated rotateInDownLeft custo-rotateInDownLeft" role="dialog">
                                <div class="modal-dialog">
                                    <!-- Modal content-->
                                     <form action="{{ route('statizin.destroy',$al->id) }}" method="POST">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                    <p class="modal-text">Anda yakin ingin menghapus {{ $al->title }}</p>
                                            </div>
                                            <div class="modal-footer md-button">
                                                <button class="btn" data-dismiss="modal"><i class="flaticon-cancel-12"></i> Discard</button>
                                                @csrf
                                                @method('DELETE')
                                                @can('mod-statistikperizinan-delete')
