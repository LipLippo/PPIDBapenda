    ...
    <div class="row layout-top-spacing">
            ...
            <div class="statbox widget box box-shadow">
                ...
                <div class="widget-content widget-content-area">
                   {!! Form::model($modal, ['method' => 'PATCH','route' => ['ppidmodal.update', $modal->id],'enctype'=>'multipart/form-data']) !!}
                        {{ Form::hidden('id') }}
                        ...
                        <div class="form-group mb-2 mt-0">
                            ...
                            <img width="150px" src="{{ url('packages/upload/ppid/modal/'.$modal->image) }}"/>
                             
                        </div>
                        <div class="form-group mb-4">
                            <label for="inputCity">Status</label>
                            <select class="form-control  basic" name="publish" required>
                                <option {{ $modal->publish == 1 ? 'selected' : '' }} value="1">Publish</option>
                                <option {{ $modal->publish == 0 ? 'selected' : '' }} value="0">Tidak Publish</option>
                            </select>
                        </div>
                       
                        <input type="submit" name="" class="mt-4 mb-4 btn btn-primary">
                        <a class="mt-4 mb-4 btn btn-secondary" href="{{ route('ppidmodal.index') }}"> Back</a>
                    {!! Form::close() !!}
                </div>
            </div>
        </div>
