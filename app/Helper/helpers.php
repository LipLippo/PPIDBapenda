...
function showmenuatas_ppid($id = '0')
{
    ...
    foreach ($rs->get() as $item) {
        ...
        if ($item->jenis_konten == 0) {
            ...
            echo "<li data-sm-reverse='true'  class='dropdown'><a  class='submenu-link' href='#'>" . $item->title . " <i class='bi bi-chevron-down'></i></a>";
        } else {
            // echo "<li><a href='$url' $target>".(($rolebahasa == 'en')?$item->title_english:$item->title)."</a>";
            echo "<li><a class='submenu-link' href='$url' $target>" . $item->title . "</a>";
        }
        $j++;
...
function showmenuatas2_ppid($id = '0')
{
    ...
    foreach ($rs->get() as $item) {
        ...
        if ($item->jenis_konten == 0) {
            // echo "<li data-sm-reverse='true'  class='dropdown'><a href='#'>".(($rolebahasa == 'en')?$item->title_english:$item->title)." <i class='bi bi-chevron-down'></i></a>";
            echo "<li data-sm-reverse='true'  class='dropdown'><a class='submenu-link' href='#'>" . $item->title . " <i class='bi bi-chevron-right'></i></a>";
        } else {
            // echo "<li><a href='$url' $target>".(($rolebahasa == 'en')?$item->title_english:$item->title)."</a>";
            echo "<li><a class='submenu-link' href='$url' $target>" . $item->title . "</a>";
        }
...
function showmenuatas2($id = '0')
{
    ...
    foreach ($rs->get() as $item) {
        ...
        if ($item->jenis_konten == 0) {
            // echo "<li data-sm-reverse='true'  class='dropdown'><a href='#'>".(($rolebahasa == 'en')?$item->title_english:$item->title)." <i class='bi bi-chevron-down'></i></a>";
            echo "<li data-sm-reverse='true'  class='dropdown' data-menu='$id'><a class='submenu-link' href='#'>" . $item->title . " <i class='bi bi-chevron-right'></i></a>";
        } else {
            // echo "<li><a href='$url' $target>".(($rolebahasa == 'en')?$item->title_english:$item->title)."</a>";
            echo "<li><a class='submenu-link' href='$url' $target>" . $item->title . "</a>";
        }
...
function showmenuatas2_uppd($id = '0', $role_id = 0, $url_uppd = '')
{
...
{
    return Album::leftjoin('users', 'users.id', '=', 'album.user_id')
        ->where('flag', 1)
        ->select('album.*', 'users.name')
        ->groupBy('album.id')
...
{
    return Video::leftjoin('users', 'users.id', '=', 'video.user_id')
